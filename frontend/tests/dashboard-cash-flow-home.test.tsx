...
describe('CASH-4A — Home carrega MonthlyCashFlow sem trocar KPIs', () => {
  ...
  it('preserva situation nos endpoints legados e não a envia ao cash-flow', async () => {
    ...
    await waitFor(() => {
      ...
      expect(getMonthlyExpenses).toHaveBeenCalledWith(null, null, 'open', null);
      expect(getMonthlyCashFlow).toHaveBeenCalledWith(null, null, null);
    });
    for (const args of getMonthlyCashFlow.mock.calls) {
      expect(args).not.toContain('open');
  ...
  it('ainda renderiza os KPIs de competência, não o billing de caixa', async () => {
    await renderReadyDashboard();
    await waitFor(() => {
      expect(getMonthlyRevenue).toHaveBeenCalled();
    ...
    expect(kpiScope('A receber').getByText(/R\$\s*6.000,00/)).toBeTruthy();
    expect(kpiScope('Despesas').getByText(/R\$\s*100,00/)).toBeTruthy();
    expect(kpiScope('Faturamento').queryByText(/R\$\s*999.999,99/)).toBeNull();
    expect(screen.queryByText(/R\$\s*888.888,88/)).toBeNull();
    await waitFor(() => {
      expect(document.querySelector('[data-cash-flow-state="ready"]')).toBeTruthy();
    });
  });

  it('falha do cash-flow não derruba a Home legada', async () => {
