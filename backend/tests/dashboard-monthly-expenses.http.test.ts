...
describe('GET /dashboard/monthly-expenses', () => {
  it('rejeita sem sessão', async () => {
    const app = await buildTestApp();
    const response = await app.inject({ method: 'GET', url: '/dashboard/monthly-expenses' });
  ...
  it('inclui OPEN, parcial e PAID do mês; exclui outro mês, nulo e RENEGOTIATED', async () => {
    ...
    const body = ok.json();
    expect(body.payables.total).toBe('30000');
    expect(body.payables.paid).toBe('14000');
    expect(body.payables.outstanding).toBe('16000');
    expect(typeof body.payables.items[0].amount).toBe('string');
    expect(typeof body.payables.items[0].paid).toBe('string');
    ...
    const hijack = await app.inject({
      method: 'GET',
      url: `/dashboard/monthly-expenses?tenantId=${b.tenant.id}`,
      headers: { cookie },
    });
  ...
  it('month=2026-08 filtra competência; inválido retorna 400', async () => {
    ...
    expect(current.json().payables.total).toBe('100');

    const explicit = await app.inject({
      method: 'GET',
      url: `/dashboard/monthly-expenses?month=${monthKey}`,
      headers: { cookie },
    });
    expect(explicit.statusCode).toBe(200);
    expect(explicit.json().payables.total).toBe('100');

    const prevKey = `${previousMonth.getUTCFullYear()}-${String(previousMonth.getUTCMonth() + 1).padStart(2, '0')}`;
    const past = await app.inject({
      method: 'GET',
      url: `/dashboard/monthly-expenses?month=${prevKey}`,
      headers: { cookie },
    });
    expect(past.statusCode).toBe(200);
    expect(past.json().payables.total).toBe('50');

    const futureKey = `${nextMonth.getUTCFullYear()}-${String(nextMonth.getUTCMonth() + 1).padStart(2, '0')}`;
    const future = await app.inject({
      method: 'GET',
      url: `/dashboard/monthly-expenses?month=${futureKey}`,
      headers: { cookie },
    });
    expect(future.statusCode).toBe(200);
    expect(future.json().payables.total).toBe('70');

    const invalid = await app.inject({
      method: 'GET',
      url: '/dashboard/monthly-expenses?month=2026-13',
      headers: { cookie },
    });
    ...
    const both = await app.inject({
      method: 'GET',
      url: `/dashboard/monthly-expenses?month=${monthKey}&tenantId=x`,
      headers: { cookie },
    });
  ...
  it('SUPER_ADMIN em Support Mode lê o tenant operacional', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.json().payables.total).toBe('12.5');
    expect(response.json().payables.paid).toBe('12.5');
  });
});
