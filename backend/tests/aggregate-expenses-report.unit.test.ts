...
import type { MonthlyCompetenceExpenseResult } from '../src/modules/analytics/domain/types.js';
import { aggregateExpensesReport } from '../src/modules/reports/domain/aggregate-expenses-report.js';

const ZERO = new Prisma.Decimal(0);

function month(
  monthKey: string,
  overrides: Partial<MonthlyCompetenceExpenseResult> = {},
...
describe('aggregateExpensesReport', () => {
  it('soma totais mensais e recalcula cobertura D9 no intervalo', () => {
    const result = aggregateExpensesReport([
      month('2026-01'),
      month('2026-02', {
  ...
  it('retorna coverageRate null quando o total do intervalo é zero (D9)', () => {
    ...
    expect(aggregateExpensesReport([empty]).coverageRate).toBeNull();
  });

  it('propaga cash split indisponível (CC1) para o intervalo sem somar paid', () => {
    const result = aggregateExpensesReport([
      month('2026-01'),
      month('2026-02', {
