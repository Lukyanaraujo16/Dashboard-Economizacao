...
function payable(input: {
  ...
  return {
    ...
    description: input.description ?? 'Aluguel',
...
describe('GET /dashboard/payables/expected-details', () => {
  it('rejeita sem sessão e tenantId na query', async () => {
    const app = await buildTestApp();
    const unauth = await app.inject({
      method: 'GET',
      url: '/dashboard/payables/expected-details',
    });
    expect(unauth.statusCode).toBe(401);

    const seeded = await seedConnected('epd-auth');
    await createUser({ email: 'user-epd@mcf.test', role: 'USER', tenantId: seeded.tenant.id });
    const cookie = await loginAs(app, 'user-epd@mcf.test');
    const hijack = await app.inject({
      method: 'GET',
      url: `/dashboard/payables/expected-details?tenantId=${seeded.tenant.id}`,
      headers: { cookie },
    });
  ...
  it('serializa itens, party e reconcilia com monthly-cash-flow', async () => {
    ...
    const seeded = await seedConnected('epd-main');
    const other = await seedConnected('epd-other');
    const scope = {
    ...
    await financial.upsertPayables(scope, [
      payable({
        externalId: 'open-ap-1',
        dueDate: to,
        unpaid: '1250',
        externalPartyId: 'fornecedor-1',
        categoryExternalIds: ['cat-contab'],
        description: 'Honorários contábeis',
      }),
    ]);
    await financial.upsertPayables(scopeOther, [
      payable({ externalId: 'secret-ap', dueDate: to, unpaid: '99999' }),
    ]);

    await createUser({ email: 'user-epd-main@mcf.test', role: 'USER', tenantId: seeded.tenant.id });
    const app = await buildTestApp();
    const cookie = await loginAs(app, 'user-epd-main@mcf.test');
    ...
    const details = (
      await app.inject({
        method: 'GET',
        url: `/dashboard/payables/expected-details?month=${monthKey}`,
        headers: { cookie },
      })
    ).json();

    expect(details.available).toBe(true);
    expect(details.total).toBe(cash.expected.payables);
    expect(details.items).toHaveLength(1);
    expect(details.items[0].supplierName).toBe('Fornecedor XYZ');
    expect(details.items[0].description).toBe('Honorários contábeis');
    expect(details.items[0].categoryNames).toEqual(['Contabilidade']);
    expect(details.items[0].amount).toBe('1250');
    ...
    expect(details.items.some((item: { externalId: string }) => item.externalId === 'secret-ap')).toBe(
      false,
    );
  });
});
