...
const PLATFORM_ASSET_KEY_PATTERN =
  /^platform\/branding\/(logo|favicon)\/[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.(png|jpe?g|webp)$/i;
...
export function isSafeStorageKey(storageKey: string): boolean {
  ...
  return TENANT_LOGO_KEY_PATTERN.test(storageKey) || PLATFORM_ASSET_KEY_PATTERN.test(storageKey);
}
